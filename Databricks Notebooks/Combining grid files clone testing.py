...
#     ]
    ...
    for i in range(n_chunks):
        ...
        if start == end:
            ...
            start += side_length
        print(f"levels for loop, levesl = {levels}\n")
        for level in levels:
            print(f"level: {level}\n")
...
#                            point_group]
            for foo in range(0, len(point_group), step):
                ...
                if all([p[1] == magic_value for p in current_points]):
                    print(f"all current points = magic value, level: {level}\n")
                    continue
                points_level.append((
                    statistics.mean(
            ...
            if len(points_level) == 0:
                print(f"Did not find points for chunk: {chunk_id}, level: {level}")
                continue
            ...
            if level == 0:
                ...
                #DEBUGGING
                print(f'cubes_level : {cubes_level}\n')
                cubes_level.show()
                print(f'vertices_lengths: {vertices_lengths}\n')
                vertices_lengths.show()
...
df = pd.read_parquet('/dbfs/mnt/lsde/group02/ahn3_avg_grids/avg_grid_C_01CZ1.LAZ.gzip')
...
df['diff'] = df['diff'].where(df['diff'].between(mean-4*std, mean+4*std)).ffill()
df.hist(column='diff')
...
discrete_colors = [(255, 0, 20), (255, 70, 65), (255, 128, 110), (255, 181, 165), (255, 255, 255),
    (255, 255, 255), (255, 255, 255), (124, 128, 217), (102, 107, 216), (69, 76, 215), (33, 33, 245)]
discrete_colors = [(r/255., g/255., b/255.) for r, g, b in discrete_colors]         

my_colormap = colors.ListedColormap(discrete_colors)
plt.pcolor(square_df, cmap=my_colormap)
plt.colorbar()
plt.show()
...
# Make data.
X = np.arange(3100, 3199, 10)
Y = np.arange(12000, 12124, 10)
print(X)
Z = [get_height(ahn3_test, x, y) if get_height(ahn3_test, x, y) is not None else -100 for x in X for y in Y]

#Z = np.reshape(Z, (-1, len(X)))

im = plt.imshow(Z, cmap='hot')
plt.colorbar(im, orientation='horizontal')
plt.xlim([3100, 3199])
plt.ylim([12000, 12124])
plt.show()

# COMMAND ----------

# Make data.
X = np.arange(270, 5567, 20)
Y = np.arange(6136, 12306, 20)

Z = [get_height(df_ahn2, x, y) if get_height(df_ahn2, x, y) is not None else -100 for x in X for y in Y]
Z = np.reshape(Z, (-1, len(X)))
...
plt.pcolor(square_df, cmap=my_colormap)
plt.colorbar()
plt.title("Resulting Tiles and Height Differences")
plt.show()
...
sub_df = square_df.iloc[5790:6010, 2500:3100]
my_colormap = colors.ListedColormap(discrete_colors)
plt.pcolor(sub_df, cmap=my_colormap)
plt.colorbar()
plt.title("Terschelling: Tile Height Differences Between AHN2 and AHN3")
plt.show()

# COMMAND ----------

sub_df = square_df.iloc[5950:6075, 3070:3370]
my_colormap = colors.ListedColormap(discrete_colors)
plt.pcolor(sub_df, cmap=my_colormap)
plt.colorbar()
plt.title("Ameland: Tile Height Differences Between AHN2 and AHN3")
